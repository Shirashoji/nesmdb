from __future__ import print_function
from setuptools import setup
from setuptools.command.install import install
from setuptools.command.develop import develop

import os
import shutil
import subprocess
import tarfile
import sys
from urllib.request import urlopen

try:
  # python 2
  from urllib import urlretrieve
except:
  # python 3
  from urllib.request import urlretrieve


def _build_vgm_play(build_temp, build_lib):
  # Get temp dir
  made_build_temp = False
  if not os.path.isdir(build_temp):
    os.makedirs(build_temp)
    made_build_temp = True

  # Download VGMPlay 0.40.9
  print('Downloading VGMPlay')
  tgz_filepath = os.path.join(build_temp, '0.40.9.tar.gz')
  url = 'https://github.com/vgmrips/vgmplay-legacy/archive/refs/tags/0.40.9.tar.gz'

  with urlopen(url) as response, open(tgz_filepath, 'wb') as out_file:
      data = response.read()
      out_file.write(data)

  # Extract
  print('Extracting VGMPlay')
  with tarfile.open(tgz_filepath, 'r:gz') as f:
    f.extractall(build_temp)
  vgmplay_dir = os.path.join(build_temp, 'vgmplay-0.40.9', 'VGMPlay')
  if not os.path.isdir(vgmplay_dir):
    print('WARNING: Could not extract VGMPlay')

  # Modify makefile
  makefile_fp = os.path.join(vgmplay_dir, 'Makefile')
  with open(makefile_fp, 'r') as f:
    makefile = f.read()
  # uncomment macosx flag if os is darwin
  if sys.platform == 'darwin':
    makefile = makefile.replace('#MACOSX = 1', 'MACOSX = 1')
  # only use libao on darwin
  else:
    makefile = makefile.replace('USE_LIBAO = 1', '#USE_LIBAO = 1')
  with open(makefile_fp, 'w') as f:
    f.write(makefile)

  # Remove redefined function in VGMPlay.c to enable builds on OSX
  vgmplay_fp = os.path.join(vgmplay_dir, 'VGMPlay.c')
  with open(vgmplay_fp, 'r') as f:
    source = f.read()
  source = source.replace('int clock_gettime', 'int unused_clock_gettime')
  with open(vgmplay_fp, 'w') as f:
    f.write(source)

  # Build
  print('Building VGMPlay')
  command = 'make -C {} vgm2wav'.format(vgmplay_dir)
  res = subprocess.call(command.split())
  if res > 0:
    print('WARNING: Error building VGMPlay')

  # Copy binary to build dir
  bin_fp = os.path.join(vgmplay_dir, 'vgm2wav')
  if not os.path.isfile(bin_fp):
    raise Exception('Could not find binary')
  shutil.copy(bin_fp, build_lib)

  # Cleanup
  if made_build_temp:
    shutil.rmtree(build_temp)
  else:
    os.remove(tgz_filepath)
    shutil.rmtree(os.path.join(build_temp, 'vgmplay-0.40.9'))


class VGMPlayDevelop(develop):
  def run(self):
    develop.run(self)
    install_dir = os.path.join(self.install_dir, 'nesmdb')
    _build_vgm_play('/tmp/build_vgmplay', install_dir)


class VGMPlayInstall(install):
  def run(self):
    install.run(self)
    install_dir = os.path.join(self.install_lib, 'nesmdb')
    _build_vgm_play('/tmp/build_vgmplay', install_dir)


setup(
    name='nesmdb',
    version='0.1.8',
    description='The NES Music Database (NES-MDB). Use machine learning to compose music for the Nintendo Entertainment System!',
    author='Chris Donahue',
    author_email='cdonahue@ucsd.edu',
    url='https://github.com/chrisdonahue/nesmdb',
    download_url='https://github.com/chrisdonahue/nesmdb/releases',
    license='MIT',
    packages=['nesmdb', 'nesmdb.vgm', 'nesmdb.score'],
    keywords='music nes mir midi',
    python_requires='>=2.7',
    install_requires=[
      'numpy >= 1.7.0',
      'scipy >= 1.0.0',
      'Pillow >= 5.1.0',
      'six>=1.13.0',
      'tqdm >= 4.19.9',
    ],
    test_suite='nose.collector',
    tests_require=[
      'nose >= 1.3.7',
      'pretty_midi >= 0.2.8',
      'librosa >= 0.6.1',
    ],
    cmdclass = {
      'develop': VGMPlayDevelop,
      'install': VGMPlayInstall,
    },
    entry_points = {
      'console_scripts': [
        'nesmdb_convert = nesmdb.convert:main',
      ],
    },
)
